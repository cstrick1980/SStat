  ...
  while (fgets(buffer, 255, fp) != NULL) {
     ...
     while ( (tokptr = strtok (str, delimit)) != NULLCHARPTR ) {
        ...
        if (chk == 1) {
           ...
           /* Note, since rows=0 for 1st rows, val of col doesn't matter */

          SSData->PutData(dval, SSData->row, j);
          if (CountColumns == 0) SSData->col++;
          j++;
       }

       if (chk == 0) {
          fprintf(stderr,"Bad input for data[%d,%d]|", SSData->row,
             SSData->col);
          SSData->PutData(-1114113.0, SSData->row, j);
